...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_dim=416):
        ...
        self.bbox_attrs = 6 + num_classes # x,y,w,h,theta,objectness,calsses
    ...
    def forward(self, x, targets=None):
        nA = self.num_anchors
        nB = x.size(0)
        nG = x.size(2) # number of cells in row 
        stride = self.img_dim / nG # the size if each cell
        ...
        # the output produced from 1*1 Conv layer to get the desired output
        # output : Batch_Num * Anchors_num (3) * bbox_attr (6 + cls) * Feature_map (nG * nG)
        # Get outputs
        x = torch.sigmoid(prediction[..., 0])  # Center x respect to the cell
        y = torch.sigmoid(prediction[..., 1])  # Center y respect to the cell
        w = prediction[..., 2]  # Width (normalized by the diagonal)
        h = prediction[..., 3]  # Height (normalized by the diagonal)
        theta = torch.tanh(prediction[..., 4]) # theta[-90, 90]
        ...
        # The network predicts a numbers to be multiplied by anchor_w (actually a numbers for each anchor)
        pred_boxes[..., 2] = torch.exp(w.data) * anchor_w
        pred_boxes[..., 3] = torch.exp(h.data) * anchor_h
        pred_boxes[..., 4] = theta.data * 90 # theta is predicted independently of anchor box, TODO: change this?

        # the output is the un scaled unnormalized output
        output = torch.cat(
            (pred_boxes[..., :4].view(nB, -1, 4) * stride, # anchor box
             pred_boxes[..., 4].view(nB, -1, 1), # theta 
        ...
        if targets is None:
            ...
            # match every object with grid cell and anchor box (very important function)
            iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, ttheta, tconf, tcls = build_targets(
            ...
            ttheta = ttheta.type(FloatTensor)
            ...
            loss_theta = self.mse_loss(theta[obj_mask], ttheta[obj_mask])
            loss_conf_obj = self.bce_loss(pred_conf[obj_mask], tconf[obj_mask])
            loss_conf_noobj = self.bce_loss(pred_conf[noobj_mask], tconf[noobj_mask])
            loss_conf = self.obj_scale * loss_conf_obj + self.noobj_scale * loss_conf_noobj
            loss_cls = self.ce_loss(pred_cls[obj_mask], tcls[obj_mask].argmax(1))
            total_loss = loss_x + loss_y + loss_w + loss_h + loss_theta + loss_conf + loss_cls
