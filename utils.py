...
def rescale_detections(prediction, image_dim, anchors, num_classes, cuda):
    """
    Rescales detections to image dimension and feeds predictions through activation functions
    """
    batch_size = prediction.size(0)
    output_dim = prediction.size(2)
    stride =  image_dim // output_dim
    bbox_attrs = 5 + num_classes
    num_anchors = len(anchors)

    prediction = prediction.view(batch_size, bbox_attrs * num_anchors, output_dim * output_dim)
    prediction = prediction.transpose(1, 2).contiguous()
    prediction = prediction.view(batch_size, output_dim*output_dim*num_anchors, bbox_attrs)
    ...
    anchors = torch.FloatTensor([(a_h / stride, a_w / stride) for a_h, a_w in anchors])

    if cuda:
        anchors = anchors.cuda()

    # Scale width and height by anchors
    anchors = anchors.repeat(output_dim * output_dim, 1).unsqueeze(0)
